/**
 * Insere os produtos eletrônicos reais de
 * STORAGES_SAMPLE_UPLOAD/Electronics-StockX_files/ nas subcategorias corretas de
 * Electronics (ver ANALISE_MAPEAMENTO_ELECTRONICS.md). Exclui explicitamente 5
 * arquivos que são ruído de outras categorias (tênis, camiseta, mochila, placeholder).
 *
 * Diferente do lote de jerseys: eletrônico não tem grade de tamanho — 1 variante por
 * produto. Mesmas convenções de dado (preço placeholder 99.99, status=active, estoque 0,
 * SKU "DEV-{hash8}-STD").
 *
 * Pré-requisito no banco de destino: SQL_ELECTRONICS_BRANDS_AND_COLOR_SEED.sql já
 * aplicado (marcas + cor "silver").
 *
 * Trava de segurança: upload de imagem só roda com ALLOW_IMAGE_UPLOAD=1 explícito.
 *
 * Uso: npx ts-node -T -O '{"module":"commonjs"}' -r tsconfig-paths/register scripts/seed-electronics-from-storage.ts
 */
...
const SOURCE_DIR = join(__dirname, '..', 'STORAGES_SAMPLE_UPLOAD', 'Electronics-StockX_files');
const REPORT_PATH = join(__dirname, '..', 'SEED_ELECTRONICS_REPORT.json');
const PLACEHOLDER_PRICE = 99.99;

// Ruído confirmado na análise (ANALISE_MAPEAMENTO_ELECTRONICS.md §2) — não são eletrônicos.
const EXCLUDED_FILES = new Set([
  ...
  'Sprayground-Lilo-Stitch-Official-Collab-Breakout-Backpack-Multicolor.jpg',
  'Product-Placeholder-Default-20210415.jpg',
]);

const BRAND_PATTERNS: Array<{ re: RegExp; brand: string }> = [
  { re: /^apple/i, brand: 'Apple' },
  { re: /^canon/i, brand: 'Canon' },
  { re: /^fujifilm/i, brand: 'Fujifilm' },
  { re: /^meta/i, brand: 'Meta' },
  { re: /^microsoft/i, brand: 'Microsoft' },
  { re: /^nvidia/i, brand: 'NVIDIA' },
  { re: /^nintendo/i, brand: 'Nintendo' },
  { re: /^sony/i, brand: 'Sony' },
  { re: /^valves?/i, brand: 'Valve' },
  { re: /^steam/i, brand: 'Valve' },
  { re: /^teenage-engineering/i, brand: 'Teenage Engineering' },
  { re: /^finalmouse/i, brand: 'finalmouse' },
];

const CATEGORY_KEYWORDS: Array<{ re: RegExp; sub: string }> = [
  // steam-machine primeiro: é o console em si, mesmo quando o nome menciona
  // "with controller" (bundle) — não deixar o keyword genérico de controller abaixo
  // classificar errado como periférico.
  { re: /steam-machine/i, sub: 'gaming-consoles' },
  { re: /controller|pencil|keyboard|mouse|dualsense/i, sub: 'computer-gaming-peripherals' },
  { re: /graphics-card/i, sub: 'computer-components' },
  { re: /airpods/i, sub: 'audio' },
  { re: /console|xbox|switch|playstation|ps5|portal/i, sub: 'gaming-consoles' },
];
...
function detectBrandAndCollab(nameNoExt: string): { brand: string | null; brandMatchText: string | null; isCollab: boolean; rest: string } {
  // Case-sensitive de propósito: colab de verdade sempre usa "-x-" minúsculo nos nomes
  // de arquivo da StockX (ex.: "Kith-x-adidas"). "-X-" maiúsculo é parte do nome do
  // produto (ex.: "Xbox-Series-X-Console", "PowerShot-G7-X-Mark-III"), não separador.
  const segments = nameNoExt.split(/-x-/);
  const isCollab = segments.length > 1;

  if (!isCollab) {
    const match = BRAND_PATTERNS.find((p) => p.re.test(nameNoExt));
    // brandMatchText = texto que realmente casou no arquivo (ex. "Valves"), não o nome
    // de exibição (ex. "Valve") — usado depois pra cortar o prefixo certo do nome.
...
function classifyCategory(nameNoExt: string): string {
  const hit = CATEGORY_KEYWORDS.find((k) => k.re.test(nameNoExt));
  return hit ? hit.sub : 'other-electronics';
}
...
async function main() {
  ...
  for (const file of files) {
    ...
    const subSlug = classifyCategory(cleanedName);
