/**
 * Insere os produtos reais de STORAGES_SAMPLE_UPLOAD/Sports-Equipment-StockX_files/
 * em Collectibles > Sports Equipment. Exclui 4 arquivos de ruído (tênis, camiseta,
 * placeholder) que não são equipamento esportivo.
 *
 * 1 variante por produto (sem grade de tamanho), preço placeholder 99.99, estoque 0,
 * SKU "DEV-{hash8}-STD" — mesmas convenções dos lotes anteriores.
 *
 * Pré-requisito no banco de destino: SQL_SPORTS_EQUIPMENT_BRANDS_SEED.sql já aplicado.
 * Trava de segurança: upload de imagem só roda com ALLOW_IMAGE_UPLOAD=1 explícito.
 *
 * Uso: npx ts-node -T -O '{"module":"commonjs"}' -r tsconfig-paths/register scripts/seed-sports-equipment-from-storage.ts
 */
...
const SOURCE_DIR = join(__dirname, '..', 'STORAGES_SAMPLE_UPLOAD', 'Sports-Equipment-StockX_files');
const REPORT_PATH = join(__dirname, '..', 'SEED_SPORTS_EQUIPMENT_REPORT.json');
const PLACEHOLDER_PRICE = 99.99;
const TARGET_CATEGORY_SLUG = 'sports-equipment';

// Ruído confirmado (mesmo padrão dos lotes anteriores) — não são equipamento esportivo.
...
const BRAND_PATTERNS: Array<{ re: RegExp; brand: string }> = [
  ...
  { re: /^odyssey/i, brand: 'Odyssey' },
  { re: /^spalding/i, brand: 'Spalding' },
  { re: /^stanley/i, brand: 'Stanley' },
  { re: /^selkirk/i, brand: 'Selkirk' },
];
...
function stripSuffixes(nameNoExt: string): string {
  return nameNoExt.replace(/-Product$/i, '').replace(/-Sample-Image$/i, '').replace(/-[Vv]\d+$/i, '').replace(/-\d+$/i, '').replace(/-Product$/i, '');
}

function detectBrandAndCollab(nameNoExt: string): { brand: string | null; brandMatchText: string | null; isCollab: boolean; rest: string } {
  // Case-sensitive: colab de verdade usa "-x-" minúsculo (ex.: "Kith-x-TaylorMade").
